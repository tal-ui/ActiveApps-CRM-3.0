...
export const OBJECTS: Record<string, ObjectDef> = {
  ...
  accounts: {
    ...
    relatedLists: [
      ...
      { object: "invoices", foreignKey: "account_id", columns: ["invoice_number", "status", "total_amount", "balance", "due_date"] },
      { object: "recurring_invoices", foreignKey: "account_id", title: "Retainers", columns: ["name", "status", "amount", "next_run_date"] },
  ...
  invoices: {
    ...
    highlightFields: ["account_id", "status", "total_amount", "balance", "due_date"],
    ...
    fields: [
      ...
      { name: "paid_date", label: "Paid Date", type: "date", readOnly: true, section: "Dates" },
      ...
      // Rolled up from invoice_payments by the same trigger.
      { name: "amount_paid", label: "Paid", type: "currency", readOnly: true, section: "Amounts" },
      { name: "balance", label: "Balance Due", type: "currency", readOnly: true, section: "Amounts", showInList: true },
      ...
      // Written by the issue-invoice edge function once Green Invoice has
      // allocated a legal document; a DB trigger freezes the amounts after.
      { name: "external_doc_number", label: "Tax Invoice #", type: "text", readOnly: true, section: "Tax Document" },
      { name: "allocation_number", label: "Allocation # (מספר הקצאה)", type: "text", readOnly: true, section: "Tax Document" },
      { name: "external_doc_url", label: "Provider PDF", type: "url", readOnly: true, section: "Tax Document" },
      { name: "issued_at", label: "Issued At", type: "date", readOnly: true, section: "Tax Document" },
    ],
    relatedLists: [
  ...
  invoice_payments: {
    ...
    highlightFields: ["invoice_id", "amount", "paid_at", "method"],
    searchFields: ["reference"],
    fields: [
      { name: "invoice_id", label: "Invoice", type: "lookup", lookup: "invoices", required: true, section: "Payment" },
      { name: "amount", label: "Amount", type: "currency", required: true, section: "Payment", showInList: true },
      { name: "paid_at", label: "Date Received", type: "date", required: true, section: "Payment", showInList: true },
      { name: "method", label: "Method", type: "picklist", required: true, defaultValue: "bank_transfer", section: "Payment", options: opts("bank_transfer", "credit_card", "check", "cash", "paypal", "other"), showInList: true },
      { name: "reference", label: "Reference", type: "text", section: "Payment", showInList: true },
      { name: "notes", label: "Notes", type: "textarea", section: "Notes" },
    ],
  },
  ...
  recurring_invoices: {
    ...
    icon: Repeat,
    titleFields: ["name"],
    highlightFields: ["account_id", "status", "amount", "next_run_date"],
    searchFields: ["name"],
    inNav: true,
    ownerFields: ["owner_id", "created_by_id"],
    fields: [
      { name: "name", label: "Retainer Name", type: "text", required: true, section: "Retainer", showInList: true },
      { name: "account_id", label: "Account", type: "lookup", lookup: "accounts", required: true, section: "Retainer", showInList: true },
      { name: "project_id", label: "Project", type: "lookup", lookup: "projects", section: "Retainer", dependsOn: { field: "account_id", column: "account_id" } },
      { name: "status", label: "Status", type: "picklist", required: true, defaultValue: "active", section: "Retainer", options: opts("active", "paused", "ended"), showInList: true },
      { name: "amount", label: "Amount per Period", type: "currency", required: true, section: "Billing", showInList: true },
      { name: "currency", label: "Currency", type: "text", defaultValue: "ILS", section: "Billing" },
      { name: "tax_rate", label: "Tax Rate (%)", type: "number", defaultValue: 18, section: "Billing" },
      { name: "frequency", label: "Frequency", type: "picklist", required: true, defaultValue: "monthly", section: "Schedule", options: opts("monthly", "quarterly", "annual"), showInList: true },
      { name: "line_description", label: "Invoice Line Description", type: "text", section: "Billing" },
      { name: "include_unbilled_hours", label: "Also bill unbilled hours", type: "boolean", defaultValue: false, section: "Billing" },
      { name: "start_date", label: "Start Date", type: "date", section: "Schedule" },
      { name: "end_date", label: "End Date", type: "date", section: "Schedule" },
      { name: "next_run_date", label: "Next Invoice On", type: "date", section: "Schedule", showInList: true },
      { name: "last_run_date", label: "Last Generated", type: "date", readOnly: true, section: "Schedule" },
    ],
    relatedLists: [
      { object: "invoices", foreignKey: "recurring_invoice_id", title: "Generated Invoices", columns: ["invoice_number", "status", "total_amount", "balance", "issue_date"] },
    ],
  },
